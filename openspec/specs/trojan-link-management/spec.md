...
The system SHALL allow the user to input a supported Trojan share link and save it as the active connection configuration.
...
- **WHEN** the user submits a syntactically valid Trojan link containing unsupported required parameters for the current release
...
The system SHALL persist the active Trojan link across application restarts until the user replaces or clears it.

#### Scenario: Relaunch app with saved Trojan configuration
- **WHEN** the application restarts after a Trojan link has been saved
- **THEN** the system restores the saved Trojan configuration as the active connection
